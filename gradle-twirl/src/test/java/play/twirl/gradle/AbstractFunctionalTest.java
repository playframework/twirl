...
abstract class AbstractFunctionalTest {
  ...
  protected BuildResult build(String gradleVersion, String... args) {
    return runner
        ...
        // Enable configuration cache for any build
        .withArguments(ArrayUtils.add(args, "--configuration-cache"))
        .build();
  }
  ...
  static Stream<String> gradleVersions() {
    ...
    if (JavaVersion.current().compareTo(JavaVersion.VERSION_21) >= 0) { // Gradle 8.4+
      return Stream.of("8.4.0", latest);
    }
    if (JavaVersion.current().compareTo(JavaVersion.VERSION_17) >= 0) { // Gradle 8.0+
      return Stream.of("8.0.2", latest);
    }
    // https://docs.gradle.org/current/userguide/scala_plugin.html#sec:configure_zinc_compiler
    if (getScalaVersion().equals("3")) { // Gradle 8.0.2+
      return Stream.of("8.0.2", latest);
    }
    return Stream.of("8.0.2", latest);
  }
