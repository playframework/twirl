  ...
  def templateOrLocalMemberDeclaration(): (PosString, PosString, Either[Boolean, Boolean]) = {
    ...
    if (check("@")) {
      ...
      if (isLazy && isVar) {
        ...
        error("'lazy' not allowed here. Only vals can be lazy", lazypos)
        return null
      }
      if (isLazy && !isVal) {
        input.regressTo(resetPosition) // don't consume @
      ...
      if (impExp != null) imports += impExp
        ...
        val lmemberOrVarReassignment = localMember() match {
          case Def(name, params, resultType, code)
              if (previousDefinedLocalMembersInParents ++ localMembers).exists(_ match {
                case Var(varname, _, _) if name == varname => true
                case _                                     => false
              }) => {
            if (!params.str.isEmpty) {
              input.regressTo(memberPosition)
              error(
                s"$name is already defined as variable $name. To reassign $name remove the argument list(s) and type parameter(s). Otherwise choose another name.",
                memberPosition
              )
              null
            } else if (!resultType.map(_.str.isEmpty).getOrElse(true)) {
              input.regressTo(memberPosition)
              error(s"Type parameter not allowed when reassigning variable", memberPosition)
              null
            } else {
              val reassignment = Reassignment(Right(Var(name, None, code)))
              mixeds += reassignment
              reassignment
            }
          }
        ...
        if (lmemberOrVarReassignment == null) {
          ...
          val templateOrVarReassignment = template(
            ...
            case tmpl @ SubTemplate(declaration, name, params, _, _, _, _)
                if declaration.left // we only care about `@name = { ... }`, meaning no `var`, `val` or `lazy` keyword was given,
                  ...
                  (previousDefinedTemplatesInParents ++ templates).exists(_.name == name) => {
              if (!params.str.isEmpty) {
          ...
          if (templateOrVarReassignment == null) {
            val mix = mixed(previousDefinedLocalMembersInParents, previousDefinedTemplatesInParents)
