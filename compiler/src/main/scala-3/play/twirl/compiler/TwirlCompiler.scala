...
object TwirlCompiler {
  ...
  def compileVirtual(
      ...
      codec: Codec = TwirlIO.defaultCodec,
      inclusiveDot: Boolean = false
  )(using qctx: Quotes): GeneratedSourceVirtual = {
  ...
  def generatedFileVirtual(
      template: File,
      sourceDirectory: File,
      inclusiveDot: Boolean
  ): (Array[String], GeneratedSourceVirtual) = {
    val templateName = {
      val name =
        source2TemplateName(template, sourceDirectory, template.getName.split('.').takeRight(1).head).split('.')
      if (inclusiveDot) addInclusiveDotName(name) else name
  ...
  def generateCode(
      ...
      additionalImports: collection.Seq[String],
      constructorAnnotations: collection.Seq[String]
  )(using qctx: Quotes): collection.Seq[Any] = {
    val (renderCall, f, templateType): (String, String, String) =
      TemplateAsFunctionCompiler.getFunctionMapping(root.params.str, resultType)
...
"""
  ...
  object TemplateAsFunctionCompiler {
    ...
    import dotc.util.Position
    import dotc.util.BatchSourceFile
    ...
    def getFunctionMapping(signature: String, returnType: String)(using qctx: Quotes): (String, String, String) = ???
      synchronized {
        import qctx.reflect.*
        ...
        val params: DefDef =
          findSignature(PresentationCompiler.treeFrom("object FT { def signature" + signature + " }")).get
        ...
        val resp = (renderCall, f, templateType)
        ...
        resp match {
          case None =>
            report.throwError(new Throwable("Timeout in getFunctionMapping"), pos)
            ("", "", "")
          case Some(res) =>
            res match {
              case Right(t) =>
                report.throwError(new Throwable("Throwable in getFunctionMapping", t))
                ("", "", "")
              case Left(res) =>
                res
            }
    ...
    class CompilerInstance {
      ...
      private lazy val initCtx: Context = (new ContextBase).initialCtx
...
//          val origBootclasspath =
//            settings.allSettings.find(_.name == "bootclasspath").map(_.valueIn(settings.defaultState))
//
//          settings.bootclasspath.value =
//            ((origBootclasspath :: pathList) ::: additionalClassPathEntry.toList).mkString(File.pathSeparator)
        }

        val compiler = new InteractiveDriver(List())
